...
import com.zy.blog.picture.mapper.NetworkDiskMapper;
...
import com.zy.blog.picture.util.MinioUtil;
import com.zy.blog.picture.util.MoGuFileUtil;
import com.zy.blog.picture.vo.NetworkDiskVO;
...
@RefreshScope
@Service
public class NetworkDiskServiceImpl extends ServiceImplBase<NetworkDiskMapper, NetworkDisk> implements NetworkDiskService {
...
//    @Value(value = "${file.upload.path}")
    ...
    public List<NetworkDisk> selectFilePathTree() {
        ...
        queryWrapper.eq(SQLConf.ADMINUID, RequestHolder.getAdminUid());
    ...
    public List<NetworkDisk> selectFileList(NetworkDisk networkDisk) {
        ...
        // 获取配置文件
        Map<String, String> qiNiuResultMap = feignUtil.getSystemConfigMap("");
        String picturePriority = qiNiuResultMap.get(SysConf.PICTURE_PRIORITY);
        QueryWrapper<NetworkDisk> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(SysConf.STATUS, EStatus.ENABLE);
        queryWrapper.eq(SysConf.ADMIN__UID, RequestHolder.getAdminUid());
        ...
        list.forEach(item -> {
            if (EFilePriority.MINIO.equals(picturePriority)) {
