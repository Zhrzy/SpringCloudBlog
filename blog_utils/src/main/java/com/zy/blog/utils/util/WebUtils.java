...
public class WebUtils {
    ...
    public static <T> T getData(String result, Class<T> beanType) {
        if (com.zy.blog.utils.util.StringUtils.isEmpty(result)) {
            return null;
        }
        Map<String, Object> dataMap = (Map<String, Object>) com.zy.blog.utils.util.JsonUtils.jsonToObject(result, Map.class);
        if (SUCCESS.equals(dataMap.get(CODE))) {

            Map<String, Object> data = (Map<String, Object>) dataMap.get("data");
            T t = com.zy.blog.utils.util.JsonUtils.mapToPojo(data, beanType);
            return t;
        }
        return null;
    }
