  ...
  <div class="app-container">
    ...
    <el-table :data="tableData"
      ...
      <el-table-column label="推荐等级" width="100" align="center" prop="level" sortable="custom" :sort-by="['level']">
        <template slot-scope="scope">
          <el-tag v-for="item in getLevel(scope.row.level)" :key="item.uid"  :type="item.listClass">{{item.dictLabel}}</el-tag>
        </template>
      </el-table-column>
       <el-table-column label="标签" width="200" align="center" >
        <template slot-scope="scope">
          <template>
            <el-tag style="margin-left: 3px" type="warning"   :key="index"  v-for="(item, index) in getTag(scope.row.tagList)">{{item.content}}</el-tag>
          </template>
        </template>
      </el-table-column> 
      ...
      <el-table-column label="开启评论" width="100" align="center" prop="openComment" sortable="custom" :sort-by="['openComment']">
        <template slot-scope="scope">
          <template>
            <el-tag v-for="item in getOpenComment(scope.row.openComment)" :key="item.uid" :type="item.listClass" >{{item.dictLabel}}</el-tag>
          </template>
        </template>
      </el-table-column>
      ...
      <el-table-column label="发布状态" width="100" align="center" prop="isPublish" sortable="custom" :sort-by="['isPublish']">
        <template slot-scope="scope">
          <template>
            <el-tag v-for="item in getIsPublish(scope.row.isPublish)" :key="item.uid" :type="item.listClass">{{item.dictLabel}}</el-tag>
          </template>
        </template>
      </el-table-column> 
    ...
    >
      <el-form :model="form" :rules="rules" ref="form">
        ...
        <el-form-item label="内容" :label-width="formLabelWidth" prop="content">
         ...
         <MarkdownEditor  :content="form.content" ref="editor" :height="1465"></MarkdownEditor>
        </el-form-item>
  ...
  computed:{
    getLevel(){
      return function(param){  
        return this.blogLevelDictList.filter(e=>{ return e.dictValue==param });
    ...
    getOpenComment(){
      return (param)=>{
       return this.openDictList.filter(e=>{ return e.dictValue==param });
      }
    },
    getIsPublish(){
      return (param)=>{
       return this.blogPublishDictList.filter(e=>{ return e.dictValue==param });
      }
    }
  },
  data() {
    ...
    getSystemConfigList: function() {
      getSystemConfig().then(response => {
        if (response.code == this.$ECode.SUCCESS) {
          ...
          this.$commonUtil.message.error(response.data);
        }
      });
    },
    getChooseData(data) {
