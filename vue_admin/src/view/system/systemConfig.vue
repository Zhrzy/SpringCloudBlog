      ...
      <el-tab-pane name="one" v-permission="'/systemConfig/getSystemConfig'">
        ...
        <el-form style="margin-left: 20px;" label-position="left"   label-width="140px" >
          ...
          <el-form-item label="图标类型">
            <el-radio v-for="item in iconConfigList" :key="item.uid" v-model="form.iconType" :label="item.dictValue" border size="medium">{{item.dictLabel}}</el-radio>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getDictList: function () {
      ...
      var dictTypeList =  ['sys_search_model', 'sys_yes_no', 'sys_picture_priority', 'sys_storage_region', 'sys_normal_disable', 'sys_editor_modal','icon_type']
    ...
    submitForm: function() {
      ...
      editSystemConfig(this.form).then(res => {
            if (res.code == this.$ECode.SUCCESS) {
               //vuex设置系统icon类型
          this.$store.dispatch('settings/changeIconType',this.form.iconType).then(()=>{
          });
