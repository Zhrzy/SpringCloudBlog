...
import com.zy.blog.utils.enums.ECommentType;
...
public class CommentRestApi {
   ...
   /* @Value(value = "${BLOG.USER_TOKEN_SURVIVAL_TIME}")*/
    ...
    public String getListByApp(@Validated({GetList.class}) @RequestBody CommentView commentVO, BindingResult result) {
    ...
    public String getListByUser(HttpServletRequest request, @Validated({GetList.class}) @RequestBody UserView userVO) {
    ...
    public String add(@Validated({Insert.class}) @RequestBody CommentView commentVO, BindingResult result) {
        ...
        if (commentVO.getContent().length() > SysConf.ONE_ZERO_TWO_FOUR) {
            return ResultUtil.result(SysConf.ERROR, com.zy.blog.web.global.MessageConf.COMMENT_CAN_NOT_MORE_THAN_1024);
        }
        // 判断该用户是否被禁言
        if (user.getCommentStatus() == SysConf.ZERO) {
            return ResultUtil.result(SysConf.ERROR, com.zy.blog.web.global.MessageConf.YOU_DONT_HAVE_PERMISSION_TO_SPEAK);
        }
        // 判断是否发送过多无意义评论
        ...
        if (!StringUtils.isEmpty(jsonResult)) {
            ...
            if (count >= Constants.NUM_FIVE) {
                return ResultUtil.result(SysConf.ERROR, com.zy.blog.web.global.MessageConf.PLEASE_TRY_AGAIN_IN_AN_HOUR);
            }
        }
        // 判断是否垃圾评论
        ...
        if (StringUtils.isCommentSpam(content)) {
            ...
            return ResultUtil.result(SysConf.ERROR, com.zy.blog.web.global.MessageConf.COMMENT_IS_SPAM);
        }
    ...
    @PostMapping("/report")
    public String reportComment(HttpServletRequest request, @Validated({GetOne.class}) @RequestBody CommentView commentVO, BindingResult result) {
        ...
        if (comment.getUserUid().equals(commentVO.getUserUid())) {
            return ResultUtil.result(SysConf.ERROR, com.zy.blog.web.global.MessageConf.CAN_NOT_REPORT_YOURSELF_COMMENTS);
        }
        // 查看该用户是否重复举报该评论
        ...
        if (commentReportList.size() > 0) {
            return ResultUtil.result(SysConf.ERROR, com.zy.blog.web.global.MessageConf.CAN_NOT_REPEAT_REPORT_COMMENT);
        }
    ...
    @PostMapping("/delete")
    public String deleteBatch(HttpServletRequest request, @Validated({Delete.class}) @RequestBody CommentView commentVO, BindingResult result) {
     ...
     * @return
     */
    private String getUrlByCommentSource(CommentView commentVO) {
