...
import com.zy.blog.entity.*;
import com.zy.blog.service.mapper.BlogMapper;
import com.zy.blog.service.mapper.BlogSortMapper;
import com.zy.blog.service.mapper.TagMapper;
...
import com.zy.blog.utils.constant.EntityConstant;
import com.zy.blog.utils.constant.PublicConstant;
import com.zy.blog.utils.enums.*;
...
@Service
@Slf4j
public class BlogServiceImpl extends ServiceImplBase<BlogMapper, Blog> implements BlogService {
    ...
    @Override
    public IPage<Blog> getBlogList(BlogView blogView) {
        QueryWrapper<Blog> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(EntityConstant.STATUS, "1");
        Page<Blog> page = new Page<>();
        page.setCurrent(blogView.getCurrentPage());
        page.setSize(blogView.getPageSize());
        IPage<Blog> blogIPageList = blogService.page(page, queryWrapper);
        List<Blog> blogList = blogIPageList.getRecords();
        //中间层处理
        if (blogList.size() == 0) {
            return blogIPageList;
        }
        //字典类数据uid
        List<String> sortUid = new ArrayList<>();
        List<String> tagUid = new ArrayList<>();
        blogList.stream().forEach(item->{
            sortUid.add(item.getBlogSortUid());
            tagUid.addAll(StringUtils.stringToList(item.getTagUid(), PublicConstant.COMMA));
        });

        //获取数据集合,转换为map <key,value> -> <uid,Object>
        Map<String, BlogSort> sortMap =new HashMap<>();
        Map<String, Tag> tagMap =new HashMap<>();
        if(sortUid.size() > 0) {
            sortMap = blogSortService.listByIds(sortUid).stream().collect(Collectors.toMap(BlogSort::getUid, blogSort -> blogSort));
        }
        if (tagUid.size() > 0) {
            tagMap = tagService.listByIds(tagUid).stream().collect(Collectors.toMap(Tag::getUid, tag -> tag));
        }
        //填充
        for (Blog blog :blogList){
            ...
            if (StringUtils.isNotEmpty(blogSortUid))
                blog.setBlogSort(sortMap.get(blogSortUid));
            //博客标签
            List<String> uidList = StringUtils.stringToList(blog.getTagUid(), PublicConstant.COMMA);
            List<Tag> tagList = new ArrayList<>();
            Map<String, Tag> finalTagMap = tagMap;
            uidList.stream().forEach(uid->{
                 tagList.add(finalTagMap.get(uid));
            });
    ...
    @Override
    public IPage<Blog> getPageList(BlogView blogVO) {
        QueryWrapper<Blog> queryWrapper = new QueryWrapper<>();
        // 构建搜索条件
    ...
    @Override
    public String addBlog(BlogView blogVO) {
    ...
    @Override
    public String editBlog(BlogView blogVO) {
        HttpServletRequest request = RequestHolder.getRequest();
    ...
    @Override
    public String editBatch(List<BlogView> blogVOList) {
        if (blogVOList.size() <= 0) {
            return ResultUtil.errorWithMessage(MessageConf.PARAM_INCORRECT);
        }
        List<String> blogUidList = new ArrayList<>();
        Map<String, BlogView> blogVOMap = new HashMap<>();
        ...
        blogList.forEach(blog -> {
            BlogView blogVO = blogVOMap.get(blog.getUid());
            if (blogVO != null) {
                blog.setAuthor(blogVO.getAuthor());
                blog.setArticlesPart(blogVO.getArticlesPart());
                blog.setTitle(blogVO.getTitle());
    ...
    @Override
    public String deleteBlog(BlogView blogVO) {
        Blog blog = blogService.getById(blogVO.getUid());
    ...
    @Override
    public String deleteBatchBlog(List<BlogView> blogVoList) {
        if (blogVoList.size() <= 0) {
            return ResultUtil.errorWithMessage(MessageConf.PARAM_INCORRECT);
        }
...
//        }
    ...
    private void updateSolrAndRedis(Boolean isSave, Blog blog) {
        ...
        if (isSave && EPublish.PUBLISH.equals(blog.getIsPublish())) {
            ...
            //发送到RabbitMq
            rabbitTemplate.convertAndSend(SysConf.EXCHANGE_DIRECT, SysConf.MOGU_BLOG, map.toString());
